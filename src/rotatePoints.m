...
%ROTATEPOINTS
%
%   Rotate the 'originalData' in the form of Nx2 or Nx3 about the origin by
%   aligning the x-axis with the alignment vector
%
%   Rdata = rotatePoints([1,2,-1], [Xpts(:), Ypts(:), Zpts(:)])
%       rotates the (X,Y,Z) pts in 3D with respect to the vector [1,2,-1]
%
%   Rotating using spherical components can be done by first converting 
%   using:
%       [dX,dY,dZ] = cart2sph(theta, phi, rho);
%       alignmentVector = [dX,dY,dZ];
%
% Example:
%   Rotate the point [3,4,-7] with respect to the following:
%   Original associated vector is always [1,0,0]
%   Calculate the appropriate rotation requested with respect to the 
%   x-axis. For example, if only a rotation about the z-axis is
